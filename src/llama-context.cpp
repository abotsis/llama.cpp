...
size_t llama_context::state_seq_get_size(llama_seq_id seq_id, llama_state_seq_flags flags, llama_pos p0, llama_pos p1) {
    if ((p0 != -1 || p1 != -1) && (flags & LLAMA_STATE_SEQ_FLAGS_ON_DEVICE)) {
        LLAMA_LOG_ERROR("%s: range-bounded state does not support ON_DEVICE\n", __func__);
        return 0;
    }
...
size_t llama_context::state_seq_set_data(llama_seq_id seq_id, const uint8_t * src, size_t size, llama_state_seq_flags flags) {
    if ((flags & LLAMA_STATE_SEQ_FLAGS_APPEND) && (flags & LLAMA_STATE_SEQ_FLAGS_ON_DEVICE)) {
        LLAMA_LOG_ERROR("%s: APPEND not supported together with ON_DEVICE\n", __func__);
        return 0;
    }
...
size_t llama_state_seq_get_size_range(llama_context * ctx, llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_state_seq_flags flags) {
